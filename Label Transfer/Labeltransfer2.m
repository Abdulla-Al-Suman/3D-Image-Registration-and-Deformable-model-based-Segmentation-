...
clear all 
close all
load('Patient 1 muscle data.mat')% contains muscle_data cell array
%load('Patient2.mat')
...
%x2=VV;
[M,N,O] = size(x1);
MN = 16; SNA = 1; FN = 24; SNP=1; % MN=muscle number, SNA=1(right)= side number atlas, FN= frame number , SNP=2(right)side number patient

xl1 = muscle_data{MN}{SNA}{FN}.x;  % muscle selection
yl1 = muscle_data{MN}{SNA}{FN}.y;
L=length(xl1);xl1 = xl1-129.5;yl1 = yl1-129.5;
...
xn = 0.9953*xl1+0.0219*yl1-0.0294*zl1-1.3955;
yn = 0.0038*xl1+0.9983*yl1-0.0356*zl1+0.8510;
zn = 0.0088*xl1-0.0091*yl1+1.2709*zl1-0.4444;
