...
clear all 
close all
load('Patient 39 muscle data.mat')
load('Patient 39 original MRI.mat')
MX = muscle_data{3}{1}{31}.x; % manual data for comparison; 1=left
MY = muscle_data{3}{1}{31}.y;
II = zeros(1024,1024);
II(:,:) = CT(:,:,31);
I = imresize(II,0.25);
...
% X = XXXX(1:LL-1);
% Y = YYYY(1:LL-1);
X =  muscle_data{3}{1}{30}.x; % without local registration "xnnnn"
Y= muscle_data{3}{1}{30}.y;  % without local registration "xnnnnn"
% imagesc(I), colormap(gray(256));
% hold on
...
%     XX(i,2) = Y(i);
% end
sigma=9;TGM =300;TGM2 = 18000; Dmax = 30;
win = 1000; % internal force waiting
